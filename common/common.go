...
// app_key
func AppKey() string {
	// 如果没有在配置文件中读到相应的键app_key对应的值
	// 就采用默认的值"godoc"
	return web.AppConfig.DefaultString("app_key", "godoc")
}

// 默认头像
func DefaultAvatar() string {
	return web.AppConfig.DefaultString("avatar", "/static/images/headimgurl.jpg")
}

// 默认封面
func DefaultCover() string {
	return web.AppConfig.DefaultString("cover", "/static/images/book.jpg")
}
